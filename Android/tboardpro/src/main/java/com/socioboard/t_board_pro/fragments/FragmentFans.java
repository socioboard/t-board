...
public class FragmentFans extends Fragment implements OnScrollListener {
	...
	Activity aActivity;
	ArrayList<ToFollowingModel> fans = new ArrayList<ToFollowingModel>();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		MainSingleTon.mixpanelAPI.track("Fragment Fans oncreate called");
			...
			if (fans.size() >= MainSingleTon.fansIds.size()) {
				...
				for (int i = fans.size(); i < (fans.size() + 99); ++i) {
					...
					if (i == fans.size()) {
						...
						userswithComma = MainSingleTon.fansIds.get(i);
	...
	protected void parseJsonResultPaged(String jsonResult) {
		...
		myprint("parseJsonResult  FragmentsFuns1");
		...
		try {
			...
			for (int i = 0; i < jsonArray.length(); ++i) {
				...
				followingModel.setFollowingStatus(false);
				...
				getActivity().runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						if (FragmentFans.this.getActivity() != null) {
							...
							fans = toFollowingAdp.tweetModels;
	...
	protected void parseJsonResult(String jsonResult) {
		...
		myprint("parseJsonResult FragmentsFuns2  ");
				...
				public void run() {
					...
					if (FragmentFans.this.getActivity() != null) {
						...
						toFollowingAdp = new ToFollowingAdapter(getActivity(),
								fans, FragmentFans.this.getActivity());
