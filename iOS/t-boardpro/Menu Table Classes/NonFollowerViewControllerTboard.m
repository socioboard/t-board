...
@interface NonFollowerViewControllerTboard ()<UITableViewDataSource,UITableViewDelegate>

@end

@implementation NonFollowerViewControllerTboard
-(void)viewDidAppear:(BOOL)animated
{
    ...
    if(runOnceFlag)
    {
    nonFollowerArr=[[NSArray alloc]initWithArray:[self getNonFollower]];
    [self fetchDataOfNonFollower];
      runOnceFlag=false;
    }
}
...
- (void)viewDidLoad
{
    ...
    runOnceFlag=true;
    offSet=20;
    nonFollowersIdList=[[NSMutableArray alloc]init];
    HUD = [[MBProgressHUD alloc] init];
    nonFollowerArr=[[NSArray alloc]initWithArray:[self getNonFollower]];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(fetchDataOfNonFollower) name:@"ReloadTimeLine" object:nil];
...
-(void)createTable
{
    self.showNonFollower=[[UITableView alloc]initWithFrame:CGRectMake(5,10,SCREEN_WIDTH-10, SCREEN_HEIGHT) style:UITableViewStylePlain];
    self.showNonFollower.delegate=self;
    self.showNonFollower.dataSource=self;
    UIView * footerView=[[UIView alloc]init];
    footerView.frame=CGRectMake(0, 0,SCREEN_WIDTH, 80);
    self.showNonFollower.tableFooterView=footerView;
    [self.view addSubview:self.showNonFollower];
}
-(NSArray*)getNonFollower
{
    ...
    NSLog(@"Followers id %@",followersIdArr);
    NSMutableArray  * nonFollowerID=[[NSMutableArray alloc]init];
    for(NSString * idFollowing in followingIdArr)
    {
        if(![followersIdArr containsObject:idFollowing])
        {
            [nonFollowerID addObject:idFollowing];
        }
    }
    return nonFollowerID;
}
-(void)fetchDataOfNonFollower
{
    [NSThread detachNewThreadSelector:@selector(showHUDLoadingView:) toTarget:self withObject:nil];
    nonFollowerCount=[NSArray arrayWithArray:nonFollowerArr];
    NSArray *smallArray;
...
//            @autoreleasepool {
                [self.showNonFollower reloadData];
...
- (void)scrollViewDidScroll:(UIScrollView *)aScrollView
{
    ...
    if(y > h + reload_distance)
    {
        ...
        {
            refresh=true;
            if(nonFollowerCount.count>offSet)
            {
...
-(void)pagingCallNextArray
{
    ...
    if(nonFollowerCount.count>offSet+20)
    {
        for (int i=offSet+1; i<=offSet+20; i++)
        {
            [smallArray addObject:[nonFollowerCount objectAtIndex:i]];
    ...
    {
        for (int i=offSet+1; i<nonFollowerCount.count; i++)
        {
            [smallArray addObject:[nonFollowerCount objectAtIndex:i]];
...
{
    NSLog(@"Follwer list id %@",[nonFollowersIdList objectAtIndex:btn.tag]);
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        @autoreleasepool {
            ...
            {
                id returned=[[FHSTwitterEngine sharedEngine]unfollowUser:[nonFollowersIdList  objectAtIndex:btn.tag] isID:YES];
                ...
                else
                {
                    dispatch_sync(dispatch_get_main_queue(), ^{
                        @autoreleasepool {
                            ...
                            [self.showNonFollower reloadData];                        }
                        [SingletonTboard updateFollowingArray:[nonFollowersIdList  objectAtIndex:btn.tag]];
                    });
