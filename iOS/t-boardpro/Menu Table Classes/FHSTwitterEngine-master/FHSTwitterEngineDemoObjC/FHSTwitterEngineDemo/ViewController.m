    ...
    if ([alertView.title isEqualToString:@"Tweet"]) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            @autoreleasepool {
                ...
                id returned =[[FHSTwitterEngine sharedEngine]getTimelineForUser:@"sukhmeetsingh1" isID:YES count:10 ];
               ...
               // [[FHSTwitterEngine sharedEngine]postTweet:tweet];
                ...
                if ([returned isKindOfClass:[NSError class]]) {
                    ...
                    NSLog(@"%@",returned);
                    title = @"Tweet Posted";
                   NSDictionary * dict=[returned objectAtIndex:0];
                    NSLog(@"data %@",dict);
                   NSLog(@"description %@",[dict objectForKey:@"id"]);
//                    message = tweet;
                }
